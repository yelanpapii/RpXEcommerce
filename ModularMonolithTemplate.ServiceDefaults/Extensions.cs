    ...
    public static TBuilder ConfigureOpenTelemetry<TBuilder>(this TBuilder builder) where TBuilder : IHostApplicationBuilder
    {
		...
		builder.Services.AddOpenTelemetry()
			.WithMetrics(metrics =>
			{
				...
				if (useOtlp)
				{
					metrics.AddOtlpExporter(opts => opts.Endpoint = new Uri(otlpEndpoint ?? string.Empty));
			...
			.WithTracing(tracing =>
			{
				...
				if (useOtlp)
				{
					tracing.AddOtlpExporter(opts => opts.Endpoint = new Uri(otlpEndpoint ?? string.Empty));
				}
			});
